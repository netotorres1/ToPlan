...
exports.update = async (req, res) => {
  ...
  try {
    if (title === '') req.body.title = 'Sem titulo'
    if (description === '') req.body.description = 'Adicionar uma descrição'
    const currentBoard = await Board.findById(boardId)
    if (!currentBoard) return res.status(404).json('Quadro não encontrado')
